...
class Layer(tf.Module, version_utils.LayerVersionSelector):
  ...
  created in various places, at the convenience of the subclass implementer:
    ...
    * in the optional `build()` method, which is invoked by the first
      `__call__()` to the layer, and supplies the TensorShape(s) of the
      input(s), which may not have been known at initialization time;
  ...
  * `call(self, inputs, *args, **kwargs)`: Called in `__call__` after making
    ...
    that could not be conveniently created in `build()`; doing so with low-level
    TensorFlow may require the use of `tf.init_scope()`.
  ...
  * `get_config(self)`: Returns a dictionary containing the configuration used
  ...
  def call(self, inputs, *args, **kwargs):  # pylint: disable=unused-argument
    ...
    wrapping TF primitives in `tf.init_scope()` where necessary).
